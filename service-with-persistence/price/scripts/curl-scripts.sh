...
'
tmpfile=/tmp/tmp.$$
echo "Creating price with payload |$json1| "
curl -X POST -d "${json1}" -H 'Content-Type: application/json' -H 'x-chenile-eid: E1' -H 'x-chenile-tenant-id: t1' localhost:8080/price > $tmpfile
cat $tmpfile | jq
id=$(cat $tmpfile | jq ".payload.id" | tr -d '"')
echo "Obtained ID = $id"
rm -f $tmpfile

# execute  "-X POST -d '${json_create}' -H 'Content-Type: application/json' localhost:8080/price"

execute  "-X GET  -H 'Content-Type: application/json' localhost:8080/price/${id}"
