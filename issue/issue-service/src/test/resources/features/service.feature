Feature: Tests the Aurora Workflow Service using a REST client. Issue service exists and is under test.
...
OPENED -(assign) -> ASSIGNED -(resolve) -> RESOLVED -(close) -> CLOSED
  ...
  Scenario: Test create issue
    ...
    And the REST response key "mutatedEntity.id" is "dummy"
	...
	Scenario: Retrieve the issue that just got created
	  When I GET a REST request to URL "/issue/dummy" 
	  Then the REST response contains key "mutatedEntity"
	  And the REST response key "mutatedEntity.id" is "dummy"
	  And the REST response key "mutatedEntity.currentState.stateId" is "OPENED"
	  And the REST response key "mutatedEntity.openedBy" is "USER1"
	...
	Scenario: Assign the issue to an assignee with comments
		When I PUT a REST request to URL "/issue/dummy/assign" with payload
		"""
		{
	 ...
	 Scenario: Resolve the issue with comments
		When I PUT a REST request to URL "/issue/dummy/resolve" with payload
		"""
		{
	 ...
	 Scenario: Close the issue with comments
		When I PUT a REST request to URL "/issue/dummy/close" with payload
		"""
		{
			"comment": "OK"
		}
